"""Unit tests for admin LLM credit / spend tracking."""
...
class LlmUsageTests(unittest.TestCase):
    ...
    def test_flat_replicate_compat_cost(self):
    ...
    def test_sync_resets_spent(self):
        ...
        state = apply_usage_event(state, provider="claude", ok=True, cost_usd=1.0)
    ...
    def test_pending_alerts_cooldown(self):
        ...
        state = apply_usage_event(state, provider="gemini", ok=True, cost_usd=1.2, now=now)
    ...
    def test_snapshot_legacy_mode(self):
    ...
    def test_token_mask_hides_secret(self):
        ...
        ident = replicate_token_identity(token, "env:TOKEN")
    ...
    def test_key_rotation_resets_spend_for_new_token(self):
        a = replicate_token_identity("r8_oldtoken_aaaaaaaa", "env:A")
        b = replicate_token_identity("r8_newtoken_bbbbbbbb", "env:B")
        state = apply_credit_sync(
            empty_state(),
