...
type InsightsPayload = {
  ...
  users_per_plan?: { plan_id: string; name: string; users: number; monthly_price_eur: number }[]
...
export function InsightsTab() {
  ...
  const planBars =
    data?.users_per_plan?.map((p) => ({
      ...
      value: p.users,
      hint: p.monthly_price_eur > 0 ? `${moneyEur(p.monthly_price_eur, 0)}/mo equiv` : 'Free',
    })) || []
  const statusBars =
    ...
    <>
      <div className="card">
        ...
        <p className="card-desc">
          Growth, plan mix, modelled subscription revenue, Viva cash, and LLM consumption. Projected
          MRR uses catalog prices (Starter €19, Premium €39, Annual €199÷12) — not auto-renewing
          invoices.
        </p>
      ...
      <div className="grid-3 insights-kpi-grid">
        <div className="stat teal">
          <div className="n">{k?.total_users ?? '—'}</div>
          <div className="l">Total users</div>
          <div className="meta">
            +{k?.new_users_7d ?? 0} / 7d · +{k?.new_users_30d ?? 0} / 30d
          </div>
        </div>
        <div className="stat">
          <div className="n">{k?.paying_active ?? '—'}</div>
          <div className="l">Paying active</div>
          <div className="meta">{k?.trial_active ?? 0} trial · {k?.pending_cancels ?? 0} pending cancel</div>
        </div>
        <div className="stat green">
          <div className="n">{moneyEur(k?.recognized_mrr_eur, 0)}</div>
          <div className="l">Projected MRR</div>
          <div className="meta">Renewing {moneyEur(k?.renewing_mrr_eur, 0)} (excl. cancel queue)</div>
        </div>
        <div className="stat">
          <div className="n">{moneyEur(k?.cash_revenue_eur, 0)}</div>
          <div className="l">Cash ({days}d)</div>
          <div className="meta">{k?.orders_count ?? 0} completed orders</div>
        </div>
        <div className="stat coral">
          <div className="n">{moneyUsd(k?.llm_cost_usd_mtd, 2)}</div>
          <div className="l">LLM cost MTD</div>
          <div className="meta">Projected month {moneyUsd(k?.projected_llm_cost_usd_month, 2)}</div>
        </div>
        <div className="stat">
          <div className="n">{k?.active_last_7d ?? '—'}</div>
          <div className="l">Active last 7d</div>
          <div className="meta">DAU today {b?.dau_today ?? 0} · near limit {k?.users_near_llm_limit ?? 0}</div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid-3">
          <div className="stat">
            <div className="n">{b?.users_with_chat ?? '—'}</div>
            <div className="l">Users with chat history</div>
            <div className="meta">Avg {b?.avg_chat_messages ?? 0} messages</div>
          </div>
          <div className="stat">
            <div className="n">{b?.total_threads ?? '—'}</div>
            <div className="l">Total threads</div>
            <div className="meta">{b?.total_memories ?? 0} memories · {b?.total_chat_messages ?? 0} messages</div>
          </div>
          <div className={`stat ${(b?.users_near_llm_limit || 0) > 0 ? 'coral' : ''}`}>
            <div className="n">{b?.users_near_llm_limit ?? '—'}</div>
            <div className="l">Near LLM cost limit</div>
            <div className="meta">≥80% of plan cost quota in window</div>
          </div>
        </div>
        <p className="meta" style={{ marginTop: 14 }}>
          Approximate contribution: {moneyEur(k?.recognized_mrr_eur, 0)} MRR (EUR) vs{' '}
          {moneyUsd(k?.projected_llm_cost_usd_month, 2)} projected LLM month (USD) — currencies differ;
